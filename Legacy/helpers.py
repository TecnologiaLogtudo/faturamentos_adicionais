...
class NotaFiscalCommonsMixin:
    ...
    def _join_block_values(self, raw_value):
        """ Junta valores de bloco em string única, separados por vírgula. """
    ...
    def _last_block_value(self, raw_value):
        """Obtém apenas o último valor de um bloco, considerando as mesmas regras de separação."""
        values = self._split_block_values(raw_value)
        if not values:
            return ""
        return values[-1]
    ...
    def _set_tag(self, tag):
        """Gerencia a lógica de tags para controle de fluxo e retomada."""
        self.check_pause()
        ...
        try:
            ...
            try:
                if valor_cte is None or str(valor_cte).strip() == '':
                    valor_float = 0.0
                elif isinstance(valor_cte, str):
                    texto = str(valor_cte).strip()
                    ...
                    valor_float = float(valor_cte)
                
                valor_formatado = f"{valor_float:.2f}".replace('.', ',')
            except ValueError:
                self.gui.log(f"Aviso: Falha ao formatar valor numérico '{valor_cte}'. Usando valor bruto.", level="warning")
                valor_formatado = str(valor_cte).replace('.', ',')
            
            locator = page.locator(selector)
            
            # Verificar se o campo não foi bloqueado pelo sistema (ex: readonly no Caminho 2)
            if locator.is_editable():
                locator.fill('')
                self.delay.custom(self.interaction_delay // 2)
                locator.type(valor_formatado, delay=self.typing_delay)
            else:
                self.gui.log("Campo Frete Valor bloqueado pela interface. Injetando valor forçadamente via JavaScript...", level="warning")
